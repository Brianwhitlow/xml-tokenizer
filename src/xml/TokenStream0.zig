...
pub fn next(self: *TokenStream) NextRet {
    ...
    defer if (self.state.depth == 0) {
        if (self.state.info == .prev_tok and switch (self.state.info.prev_tok) {
            .element_close_tag,
            .element_close_inline,
            => true,
            else => false
        }) {
            const prev_tok = self.state.info.prev_tok;
            self.state.info = .{ .trailing = prev_tok };
        }
    };
    ...
    switch (self.state.info) {
        ...
        .prev_tok => |prev_tok| switch (prev_tok) {
            .element_open => {
                self.state.depth += 1;
        ...
        .trailing => |prev_tok| switch (prev_tok) {
            .element_open => todo(),
            .element_close_tag => todo(),
            .element_close_inline => todo(),
            
            .attribute_name => todo(),
            .attribute_value_segment => todo(),
            
            .comment => todo(),
            .cdata => todo(),
            .text => todo(),
            .entity_reference => todo(),
            .whitespace => {
                std.debug.assert(self.getUtf8() == null);
                return null;
            },
...
fn returnToken(self: *TokenStream, tok: Token) NextRet {
    self.state.info = .{ .prev_tok = tok.info };
...
test "nested tags" {
    ...
    ts.reset("<foo><bar> baz <sub /></bar></foo>");
    try tests.expectElementOpen(&ts, null, "foo");
    try tests.expectElementOpen(&ts, null, "bar");
    try tests.expectText(&ts, " baz ");
...
test "comment" {
    ...
    ts.reset("<!-- -- -->");
    try tests.expectError(&ts, Error.Malformed);
    //try tests.expectNull(&ts);
...
test "depth testing" {
    ...
    ts.reset("<root>foobarbaz<sub/>\t</root> <empty/>");
    try tests.expectElementOpen(&ts, null, "root");
    try tests.expectText(&ts, "foobarbaz");
    try tests.expectElementOpen(&ts, null, "sub");
    try tests.expectElementCloseInline(&ts);
